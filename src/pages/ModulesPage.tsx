  ...
  return (
    <div className="p-6 sm:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-white/[0.06]">
        <div>
          <div className="text-xs font-mono text-slate-400 font-semibold uppercase tracking-wider mb-1 flex items-center gap-2">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-3 py-2 rounded-lg bg-white/[0.03] border border-white/[0.06] text-slate-200 text-xs font-mono placeholder:text-slate-500 focus:outline-none focus:border-emerald-500/50 w-48"
            />
          </div>
          ...
          <div className="flex items-center gap-1 p-1 rounded-lg bg-white/[0.02] border border-white/[0.06]">
            {(["all", "pending", "done"] as const).map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-3 py-1 rounded-lg text-xs font-mono transition-colors capitalize cursor-pointer ${
                  filter === f
              ...
              <div
                ...
                className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-lg border transition-all ${
                  isCurrent
                    ...
                    : "border-white/[0.06] bg-[#161c2d] hover:border-white/[0.1]"
                }`}
              >
                <div className="flex items-start sm:items-center gap-4">
                  <button
                    onClick={() => onToggleDone(moduleName, !isDone)}
                    className="mt-0.5 sm:mt-0 text-slate-500 hover:text-emerald-400 transition-colors shrink-0 cursor-pointer"
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {isDone && (
                        <span className="text-xs font-mono px-2 py-0.5 rounded-lg bg-white/[0.04] text-emerald-400 font-semibold">
                          Completed
                        </span>
                      )}
                    </div>
