...
export const ManifestView: React.FC<ManifestViewProps> = ({ status }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      {/* Header Bar */}
      <div className="rounded-2xl border border-white/[0.08] bg-[#090b10] p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
        ...
        <div className="flex items-center gap-2">
          {/* Mode Switcher */}
          <div className="flex items-center p-0.5 rounded-lg bg-white/[0.03] border border-white/[0.06] text-xs font-mono">
            <button
          ...
          <button
            onClick={handleCopy}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] text-slate-200 text-xs font-mono transition-colors"
          >
            {copied ? (
              <>
      ...
      {viewMode === "visual" ? (
          ...
          <div className="rounded-2xl border border-white/[0.08] bg-[#090b10] p-6 space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/[0.06] pb-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-8 pr-3 py-1 rounded-lg bg-white/[0.03] border border-white/[0.06] text-xs font-mono text-slate-200 placeholder:text-slate-500 focus:outline-none focus:border-emerald-500/50 w-44"
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5 pt-1">
              {modules.map(([key, isDone], i) => (
                <div
                  key={key}
                  className="flex items-center justify-between p-2.5 rounded-lg bg-white/[0.02] border border-white/[0.04] text-xs font-mono hover:bg-white/[0.04] transition-colors"
                >
        ...
        <div className="rounded-2xl border border-white/[0.08] bg-[#07090e] overflow-hidden shadow-2xl h-[550px]">
          <Editor
            height="100%"
            path="trak.json"
            language="json"
